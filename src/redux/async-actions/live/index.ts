  ...
  return (dispatch: Dispatch<AnyAction>) => {
    return getLiveIndexData().then((result) => {
      if (result.code === "1") {
        ...
        const bannerModule = moduleList.find((item) => item.module_info.title === "banner位");
        const bannerList = bannerModule ? bannerModule.list.map((banner) => ({
          id: banner.id,
          title: banner.title,
          pic: banner.pic || banner.cover,
          link: banner.link
        })) : [];
