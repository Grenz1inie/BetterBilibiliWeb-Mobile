...
class History extends React.Component<null, HistoryState> {
  ...
  private getPicUrl = (url: string) => {
    ...
    // 处理各种可能的URL格式
    // 1. 如果已经是完整的代理URL，直接返回
    if (picUrl.includes(picURL) || picUrl.startsWith('http://localhost') || picUrl.startsWith('http://127.0.0.1')) {
      return picUrl;
    }
    
    // 2. 如果是完整的https URL，移除协议后代理
    if (picUrl.startsWith('https://') || picUrl.startsWith('http://')) {
      picUrl = picUrl.replace(/^https?:/, '');
      return `${picURL}?pic=${picUrl}`;
    }
    
    // 3. 如果是协议相对路径 //i0.hdslb.com/...
                    ...
                    <div className={style.itemWrapper} key={history.aId}>
                      <a href={"/video/av" + history.aId}>
                        <div className={style.imgContainer}>
                          <img src={this.getPicUrl(history.pic)} alt={history.title} />
                        </div>
